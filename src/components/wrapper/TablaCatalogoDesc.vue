...
const estructuraEspecifica = { id: null, descripcion: '' };
const params = ref({ ...estructuraEspecifica });

const asignarObjeto = (nuevoObjeto) => {
  const propiedades = Object.keys(nuevoObjeto);
  const propiedadDescripcion = propiedades.find(propiedad => propiedad !== 'id');
  params.value = {
    ...estructuraEspecifica,
    descripcion: nuevoObjeto[propiedadDescripcion] || '',
    ...nuevoObjeto
  };
};
...
const guardar_item = () => {
   ...
   if (params.value.id) {
    ...
    const urlCat = props.urlEdit + params.value.id;
    catalogoStore.editCatalogo(urlCat, editItem, token);
   } else {
    //add user
    console.log("add");
    const urlCat = props.urlSave;
    let saveItem = {
        "descripcion": params.value.descripcion,
    }
    ...
    <div class="row d-flex justify-content-center">
      <div class="col-lg-10 col-sm-12 layout-spacing">
                <div class="d-flex justify-content-sm-end justify-content-end mb-3">
                    <a href="javascript:;" @click="edit_item(null)">
                        <svg
                            id="btn-add-contact"
            ...
            <v-client-table :data="items" :columns="thtabla" :options="table_option2">
              ...
              <template #acciones="props">
                <a href="javascript:void(0);"
                  ...
                  @click="params.id = props.row.id; edit_item(props.row);"
                >
          ...
          <div class="modal-dialog modal-md modal-dialog-centered">
            <div class="modal-content mailbox-popup">
              ...
              <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" @click="guardar_item()">{{ params.id ? 'Editar' : 'Agregar' }}</button>
              </div>
            </div>
          </div>
        </div>
