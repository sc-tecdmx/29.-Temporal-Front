...
import { veryfyOCSP, getTimeStamp } from './services.mjs';
...
async function getMimeTypeAndArrayBufferFromFile(file) {
  ...
  async function validateOCSPAndGetTimeStampPFX(certificadoBinaryBase64, originalname) {
    ...
    if(ocspResponse.status === 'Valid'){
     ...
     return {'basicResponse':ocspResponse.basicResponse, 
...
export const getCertificadoData = async (certFileData, keyFileData, pdfFilesData, password) => {
    ...
    if (certFileData.iscer === false) {
        certificadoDTO = new CertificadoDTO(certFileData, null, keyFileData, pdfFilesData, password);
        getCertificadoAndPublicKeyAndPrivateKeyPFX(certificadoDTO.pfxFileData.buffer.toString('binary'), password, payload);
